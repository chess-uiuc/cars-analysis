function [omegaQ,omegaS,omegaO,omegaROT] = raman_shift_H2(gas,molpar,maxV,maxJ)

v=[0:1:maxV+1];J=[0:1:maxJ+2];

if (isequal(gas,'H2') == 1) %special case for H2 load term energies from data file 
    load('H2_Energies_Dabrowski.mat'); %loads term energies for H2 up to v = 3 and J = 20 into variable called 'F'
    GvJ = F';clear('F');
    GvJ1 = circshift(GvJ,-1,1); %shift GvJ array -1 in v index
    omegaQ = GvJ1-GvJ;omegaQ = omegaQ(1:maxV+1,1:maxJ+1);
    omegaO = circshift(GvJ1,2,2)-GvJ;omegaO = omegaO(1:maxV+1,3:maxJ+3);
    omegaS = circshift(GvJ1,-2,2)-GvJ;omegaS = omegaS(1:maxV+1,1:maxJ+1);
    omegaROT = GvJ-circshift(GvJ,2,2);omegaROT = omegaROT(1:maxV+1,3:maxJ+3);
    return
end

%otherwise calculate the Raman frequencies from the usual expansions
