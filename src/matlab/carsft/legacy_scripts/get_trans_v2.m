function [wraman,gamma,branch,qn] = get_trans_v2(gasname,molpar,T,Tv,wexp)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Inputs:
...
wraman = []; gamma = []; branch = {}; %initialize output variables
%vi = []; Ji = []; vmax = []; Jmax = []; gas = []; Qo = [];
qn.vi = []; qn.Ji = [];qn.gas = [];qn.Qo = [];
...
for k = 1:length(gasname)
  ...
  [wQ,wS,wO,wROT] = raman_shift(gasname(kgas),molpar,vmax,Jmax);
  WRAMAN = [wQ' wS' wO' wROT'];

  %C. %%%compute the Raman linewidths in FWHM cm-1
  [gammaQ gammaS gammaO gammaROT] = meg_linewidths(Jo,T,gasname(kgas));
  ...
  vi = vi(indx);Ji = Ji(indx);gas = repmat(gasname(k),length(indx),1);


  wraman = [wraman; WRAMAN]; gamma = [gamma; GAMMA]; branch = [branch; BRANCH];
  qn.vi = [qn.vi; vi]; qn.Ji = [qn.Ji; Ji]; qn.gas = [qn.gas; gas]; qn.Qo = [qn.Qo; [vmax Jmax]];
