function [S,chi,w,p1,p2] = CARSFT_O_atom(wexp,T,P,X,dtp,dtau3,alpha,dwp) 
...
% dtp = duration of the probe pulse (ps)
...
%check for additional gas (O atom 'gas 5')
if (length(X) > length(molpar.gasname));
    maxgas = length(molpar.gasname)+1;
    Oatom = 1;XOatom = X(maxgas);X = X(1:maxgas-1);
else;
     Oatom = 0;
end;
...
%check for additional mole fraction 'gas 5' = O atom
if (Oatom == 1);
    [chiamp_Oatom,gamma_Oatom,wraman_Oatom] = get_o_atom(T,molpar);
    wraman = [wraman; wraman_Oatom'];
    gamma = [gamma; gamma_Oatom'];
    chiamp = [chiamp; XOatom*chiamp_Oatom'];
end
