function [GAMMA,gamma,DE] = meg_co(Ji,T,branch);

To=295;   %reference temperature (K)

%%CO MEG paramters from Rosasco et al., J. Chem. Phys. (1989), p. 4059

alpha=0.01337;
beta=1.462; %[1.850 1.45];%1.67;
delta=1.246;%[1.199 1.32];%1.21;
m=0.185;%0.1487;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
F=rot_terms_CO(J,0);F=squeeze(F);  %rotational term values in cm-1 in v = 0 for the specified J
...
GAMMA = 2*sum(gamma(:,:,1)); % use this for CO and commented section below for N2,O2,H2

%%%%%this part commented out because heteronuclear CO does not exhibit
%%%%%ortho/para species
...
%%%%%this part sets values of G matrix for transitions between odd and even
%%%%%J to zero (can be commented out for CO)
