function [S,chi,w,E3,w3,beta] = CARSFT_dev_CHIRP_v3(wexp,T,P,X,dtp,dtau3,alpha,dwp) 
...
% dtp = probe pulse parameters
%       dtp(1) = pulse width (ps FWHM) of probe pulse
%       dtp(2) = linewidth of Gaussian probe laser (cm-1)
%       if dtp(2) is not prescribed then probe pulse is taken as transform
%       limited
...
if (isequal(linemodel,'iso') == 1);  %default model is isolated lines in main code
    ...
    for k = 1:length(chiamp)
        waven = wraman(k)-w;indx = find(abs(waven) <= 50*gamma(k));
        chi(indx) = chi(indx) + chiamp(k)./(waven(indx) - i*gamma(k)/2);
    end
end
...
if (length(dtp) == 1);
    ...
    DW = dtp(2); %chirped pulse bandwidth in cm-1
    w3 = [-4*DW:dw:4*DW];w3 = 2*pi*c*w3*1e-12;
    beta = (DW/(2*dtp(1))).^2 - ((1e12*log(2))/(pi*3e10*dtp(1)*dtp(1))).^2;
    beta = sqrt(beta);beta = 2*pi*c*1e-12*beta;
    a = 2*log(2)/(dtp(1)*dtp(1));
    E3 = exp(-w3.*w3/(4*(a-i*beta))); E3 = E3/max(sqrt(E3.*conj(E3)));
    w3 = w3/(2*pi*c*1e-12);
end
