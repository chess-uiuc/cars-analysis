function [S,chi,w,p1,p2] = CARSFT_dev(wexp,T,P,X,dtp,dtau3,alpha,dwp) 
...
% 1)
% dwp = slit width (assumed Gaussian, in cm-1)
%
...
[wraman,gamma,qn] = get_trans(gasname,molpar,T,Tv,wexp);
gamma = P*gamma;

%compute the Raman resonant amplitude of each line
[chiamp] = get_chiamp(gasname,molpar,T,Tv,X(gasindx),qn);
...
%%%compute the probe laser E-field spectrum (assumed transform limited)
dw3 = 14.7/dtp; %probe linewidth in cm-1
w3 = [w-mean(w)];%[-5*dw3:dw:5*dw3];
E3 = exp(-2*log(2)*(w3/dw3).^2);E3 = E3.*exp(i*w3*dtau3*2*pi*3e10/1e12);

%%%include linear chirp on the probe (temporary functionality)
NT = 4; DW = NT*dw3; %NT times transform limited pulse
beta = (DW/(2*dtp)).^2 - ((1e12*log(2))/(pi*3e10*dtp*dtp)).^2;
beta = sqrt(beta);
...
%plot(time-dtau3,phi);xlim([-50 50]);pause

ww = w-mean(w);
E3a = exp(-2*log(2)*((time-dtau3)/dtp).^2);
E3a = E3a.*exp(-i*phi);

s3 = fft(ifftshift(E3));s3 = s3/sqrt(max(s3.*conj(s3)));s3 = fftshift(s3);
%plot(time,s3.*conj(s3),time,E3a.*conj(E3a),'--r','LineWidth',2);xlim([dtau3-40 dtau3+40]);pause


E3a = ifft(ifftshift(E3a));E3a = fftshift(E3a);
E3a = E3a/sqrt(max(E3a.*conj(E3a)));


%subplot(2,1,1);plot(ww,real(E3),ww,real(E3a));xlim([-2 2])
%subplot(2,1,2);plot(ww,imag(E3),ww,imag(E3a));xlim([-2 2]);pause

Sa = conv(chi,E3a,'same');Sa = Sa.*conj(Sa);
...
if (gas == 'H2');
    gamma = meg_co(J,T); %this is just to get the size of the gamma array
    %gamma = zeros(size(gamma))+0.01; %set H2 Lorentz widths to 0.01 cm-1...for now
end
