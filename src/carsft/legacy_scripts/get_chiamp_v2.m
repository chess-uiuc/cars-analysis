function [chiamp] = get_chiamp_v2(gasname,molpar,branch,T,Tv,QN);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 1:length(gasname)
  ...
  CHIAMP = zeros(1,length(QN{k}.gas));
  ...
  indxbranch = find(strcmp(QN{k}.gas,gasname(k)) == 1); %finds the rows where gas = current species
  BRANCH = branch(indxbranch);
