...
f = weights(1:length(f)).*f; %apply optional weights to emphasize some parts of spectrum more
...
evec = find(isnan(theory) == 1);theory(evec) = 0;
...
%%%%%plot the fit result if required%%%% 
    clf
    hold on
    plot(omega_grid,data/max(data),'b','linewidth',3);
    plot(omega_grid,(theory+VS)/max(data),'r--','linewidth',3);
    plot(omega_grid,(residual-offset)/max(data),'-k','linewidth',3);
    plot([min(omega_grid) max(omega_grid)],[-offset -offset]/max(data),'--k','linewidth',5)
    hold off
    lg = legend('Experiment','Theoretical','location','NorthWest');
    lg.Box = 'off';lg.FontSize = 20;
    ...
    annotation('textbox',[0.25, 0.98, 0, 0],'String',['T = ' num2str(xfit(1),'%5.0f') ' K'],'FontSize',20,'EdgeColor','none','FitBoxToText','on');
    annotation('textbox',[0.4, 0.98, 0, 0],'String',[stg2 ' = ' num2str(xfit(2),'%5.3f')],'FontSize',20,'EdgeColor','none','FitBoxToText','on');
    
    if N == 3
        annotation('textbox',[0.55, 0.98, 0, 0],'String',[stg3 ' = ' num2str(xfit(3),'%5.3f')],'FontSize',20,'EdgeColor','none','FitBoxToText','on');
    end
